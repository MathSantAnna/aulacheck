...
export function ClassRoomRollCall() {
  ...
  const [firstPeriod, setFirstPeriod] = React.useState(
    pastRollCall.firstPeriod
  );
  const [secondPeriod, setSecondPeriod] = React.useState(
    pastRollCall.secondPeriod
  );
    ...
    <Container>
      {firstPeriod && secondPeriod ? (
        <Alert style={{ marginTop: '10px' }} severity='warning'>
          A chamada para esta data já foi realizada para todos os períodos
        </Alert>
      ) : (
        <></>
      )}

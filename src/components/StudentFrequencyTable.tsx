...
export const StudentFrequencyTable = (props: any) => {
  ...
  const mutationUpdateClassRoom = useMutation({
    mutationFn: (data: { uuid: any; presence: any }) =>
      updateClassRoom(data.uuid, data.presence),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: [`GET_STUDENTS_${course?.uuid}`],
      });
      // Update the local state to reflect the changes instantly
      const updatedHistory = studentData.classroomDetails.history.map(
        (entry: any) => {
  ...
  function handleEditPresence() {
    mutationUpdateClassRoom.mutate({
      uuid: selectedClassroom.uuid,
      presence: presence,
    });
  ...
  return (
    <>
      <TableRow key={student.uuid}>
        ...
        <TableCell>
          <Link to={paths.studentsDetails.replace(':uuid', student.uuid)}>
            {student.nmstudent}
          </Link>
        </TableCell>
        <TableCell align='center'>{student.email}</TableCell>
        <TableCell align='center'>
          {student.classroomDetails.numberOfAbsent}
        </TableCell>
        <TableCell align='center'>
          <div style={{ alignItems: 'center', display: 'flex' }}>
            <Gauge
              width={80}
              height={80}
              value={student.classroomDetails.frequence}
              valueMax={100}
              startAngle={-90}
              endAngle={90}
            />
                  ...
                  {student.classroomDetails.history.map((historyRow) => (
                    <>
                      <TableRow key={historyRow.date}>
                        ...
                        <TableCell align='center'>
                          {historyRow.presence ? 'Presente' : 'Ausente'}
                        </TableCell>
                        {!isStudent && (
                          <TableCell align='center'>
                            <EditIcon
